...
def test_fail():
    try:
        OpenEXR.InputFile("non-existent")
    except:
        pass
    else:
        assert 0
...
def test_one():
    ...
    assert len(oexr.channel('R', pixel_type = FLOAT, scanLine1 = 10, scanLine2 = 10)) == (4 * (first_header['dataWindow'].max.x + 1))
...
testList.append(("test_invalid_pt", test_invalid_pt))
