...
[Examples: the base name of channel :math:`R` is :math:`R`; the base
name of channel :math:`L1.L2.R` is :math:`R`.]
...
[Examples: the layer name of channel :math:`R` is the empty string; the
layer name of channel :math:`L1.L2.R` is :math:`L1.L2`.]
...
[Examples: layer :math:`L1` encloses layers :math:`L1.L2` and
:math:`L1.L2.L3`. Layer L1 directly encloses layer :math:`L1.L2`, but
:math:`L1` does not directly enclose :math:`L1.L2.L3`.]

Alpha, Color, Depth and Auxiliary Channels 
-------------------------------------------

A channel whose base name is :math:`A`, :math:`\text{AR}`,
:math:`\text{AG}` or :math:`\text{AB}` is an **alpha channel**. All
samples must be greater than or equal to zero, and less than or equal to
one.

A channel whose base name is :math:`R`, :math:`G`, :math:`B`, or
:math:`Y` is a **color channel**.

A channel whose full name is :math:`Z` or :math:`\text{ZBack}`, is a
...
:math:`\text{ZBack}`. If the base layer does not include one, then a
:math:`\text{ZBack}` channel can be generated by copying the :math:`Z`
channel.

Layers other than the base layer may include channels called :math:`Z`
or :math:`\text{ZBack}`, but those channels are auxiliary channels and
...
:math:`i` in all other channels in the same pixel.

If :math:`S_{i}\left( Z \right) \geq S_{i}\left( \text{ZBack} \right)`,
then sample number :math:`i` in all other channels covers the single
depth value :math:`z = S_{i}\left( Z \right)`, where :math:`z` is the
distance of the sample from the viewer. Sample number :math:`i` is
called a **point sample**.

If :math:`S_{i}\left( Z \right) < S_{i}\left( \text{ZBack} \right)`,
then sample number :math:`i` in all other channels covers the half open
interval
...
:math:`c`, is found by looking for a **matching** alpha channel (see
below), first in the layer that contains :math:`c`, then in the directly
...
===================================================
\                                   
:math:`R`                            :math:`\text{AR}` if it exists, otherwise :math:`A`
:math:`G`                            :math:`\text{AG}` if it exists, otherwise :math:`A`
:math:`B`                            :math:`\text{AB}` if it exists, otherwise :math:`A`
:math:`Y`                            :math:`A`
(any auxiliary channel)              :math:`A`
====================================
===================================================
...
:math:`L1.A`     
:math:`L1.AR`    
:math:`L1.R`      :math:`L1.AR`
:math:`L1.G`      :math:`L1.A`
:math:`L1.L2.G`   :math:`L1.A`
================= ========================
...
A pixel in a deep image is **sorted** if for every :math:`i` and
:math:`j` with :math:`i` < :math:`j`,

.. math:: S_{i}\left( Z \right) < \ S_{j}\left( Z \right)\text{\ \ }\text{or}\ \left( S_{i}\left( Z \right) = S_{j}\left( Z \right)\ \text{and}\ S_{i}\left( \text{ZBack} \right) \leq S_{j}\left( \text{ZBack} \right) \right).

A pixel in a deep image is **non-overlapping** if for every :math:`i`
and :math:`j` with :math:`i \neq j`,

.. math:: \left( S_{i}\left( Z \right) < \ S_{j}\left( Z \right)\text{\ and\ }S_{i}\left( \text{ZBack} \right) \leq \ S_{j}\left( Z \right) \right)\ \text{or\ }\left( S_{j}\left( Z \right) < \ S_{i}\left( Z \right)\text{\ and\ }S_{j}\left( \text{ZBack} \right) \leq \ S_{i}\left( Z \right) \right)\ \text{or}

.. math:: \left( S_{i}\left( Z \right) = \ S_{j}\left( Z \right)\text{\ and\ }S_{i}\left( \text{ZBack} \right) \leq \ S_{i}\left( Z \right)\text{\ and\ }S_{j}\left( \text{ZBack} \right) > \ S_{j}\left( Z \right) \right)\text{\ or}

.. math:: \left( S_{j}\left( Z \right) = \ S_{i}\left( Z \right)\text{\ and\ }S_{j}\left( \text{ZBack} \right) \leq \ S_{j}\left( Z \right)\text{\ and\ }S_{i}\left( \text{ZBack} \right) > \ S_{i}\left( Z \right) \right)\text{.\ \ \ \ \ }
...
Given a color channel, :math:`c`, and its associated alpha channel,
...
:math:`z`. In other words, we divide the object into two parts by
...
are zero up to :math:`Z`, increase gradually between :math:`Z` and
:math:`\text{ZBack}`, and then remain constant.
...
Sample number :math:`i` is a volume sample; its :math:`\text{ZBack}` is
greater than its :math:`Z`. Alpha and color increase gradually between
:math:`Z` and :math:`\text{ZBack}` and then remain constant. Sample
number :math:`i + 1`, whose :math:`Z` and :math:`\text{ZBack}` are
...
Our first operation is splitting volume sample number :math:`i` of a
pixel at a given depth, :math:`z`, where\ :math:`\ `
...
=============================================================
=======================================================================================
.. math:: S_{i,new}\left( Z \right) = \ S_{i}\left( Z \right) .. math:: S_{i + 1,new}\left( Z \right) = \ z
=============================================================
=======================================================================================
.. math:: S_{i,new}\left( \text{ZBack} \right) = \ z          .. math:: S_{i + 1,new}\left( \text{ZBack} \right) = \ S_{i}\left( \text{ZBack} \right)
=============================================================
=======================================================================================

For a color channel, :math:`c`, and its associated alpha channel,
:math:`\alpha`, the new samples are:

========================================================
============================================================================================================================
.. math:: S_{i,new}\left( \alpha \right) = \alpha_{i}(z) .. math:: S_{i + 1,new}\left( \alpha \right) = \alpha_{i}({S_{i}\left( Z \right) + S}_{i}\left( \text{ZBack} \right) - z)
========================================================
============================================================================================================================
.. math:: S_{i,new}\left( c \right) = c_{i}(z)           .. math:: S_{i + 1,new}\left( c \right) = c_{i}\left( {S_{i}\left( Z \right) + S}_{i}\left( \text{ZBack} \right) - z \right)
========================================================
============================================================================================================================
...
sample in a numerically stable way, see page 15 of this document.

Merging Overlapping Samples
---------------------------

In order to make a deep image tidy, we need a procedure for merging two
samples that perfectly overlap each other. Given two samples, :math:`i`
and :math:`j`, with
...
:math:`S_{i,new}\left( c \right)` in a numerically robust way, see page
15 of this document.
...
3. Sort the samples according to :math:`Z` and :math:`\text{ZBack}` (see
   “Sorted, Non-Overlapping and Tidy Images” on page 5).
...
2. For each pixel, composite sample :math:`0` over sample :math:`1`.
   Composite the result over sample :math:`2`, and so on, until sample
   :math:`n - 1` is reached.
...
useful way to handle :math:`Z` and :math:`\text{ZBack}` depends on how
the flat image will be used. Possibilities include, among others:

-  Flatten the :math:`Z` channel as if it was a color channel, using
   :math:`A` as the associated alpha channel. For volume samples,
   replace :math:`Z` with the average of :math:`Z` and
   :math:`\text{ZBac}`\ k before flattening. Either discard the
   :math:`\text{ZBack}` channel, or use the back of the last sample,
   :math:`max\left( S_{n - 1}\left( Z \right),S_{n - 1}\left( \text{ZBack} \right) \right)`,
   as the :math:`\text{ZBack}` value for the flat image.

-  Treating :math:`A` as the alpha channel associated with :math:`Z`,
   find the depth where :math:`A(z)` becomes 1.0 and store that depth in
   the :math:`Z` channel of the flat image. If :math:`A(z)` never
...
-  Treating :math:`A` as the alpha channel associated with :math:`Z`,
   ...
   of the first opaque sample into the :math:`Z` and
   :math:`\text{ZBack}` channels of the flat image.
...
dense fog and a gray brick. For a sample that extends from :math:`Z` to
:math:`\text{ZBack}`, the function :math:`\alpha(z)` evaluates to 1.0
